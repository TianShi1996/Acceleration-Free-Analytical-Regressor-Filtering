...
ss={'-.','-'};Ts=1e-3;
if task==1
    t=0:Ts:25;filename='traj3_tau_a_1.gif';
    xind=[0,25];yind=[0,80];ind0=0;pose=[-2.5,40];
else
    t=130:Ts:155;filename='traj3_tau_a_2.gif';
    xind=[130,155];yind=[0,40];ind0=130/Ts;pose=[-2.5+130,20];
end
...
for i=1:length(t)/dur
    ...
    ylabel('{\boldmath$\|\tau\|$} (N.m)','Interpreter','latex','FontSize',ftsize,'Position',pose);
    title('The joint toruqe norms','Interpreter','latex','FontSize',ftsize);
    xlim(xind);
    ylim(yind);yticks(0:20:80);
